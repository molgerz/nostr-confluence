...
import { DEFAULT_RELAY_URL, useRelayStatus } from '../../nostr/relay-status'

export function AppShell() {
  const params = useParams<{ group?: string }>()
  const group = params.group ? parseGroupAddress(params.group) : null
  const relayUrl = group?.relayUrl ?? DEFAULT_RELAY_URL
  const status = useRelayStatus(relayUrl)

  return (
    <div className="flex h-full flex-col bg-surface-0">
      <Topbar />
      <div className="flex min-h-0 flex-1">
        <Sidebar group={group} status={status} relayUrl={relayUrl} />
